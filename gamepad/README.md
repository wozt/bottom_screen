...
| Sends a picture to a GamePad | **never run** |
| Sends sound to a GamePad | **never run** |
| Buttons, sticks and touch coming back | **never run** |

`--no-pad` runs everything except the GamePad, which is how the first
two rows were measured. It exists so that a fault in the half that can
be tested is not blamed on the half that cannot.
...
- **A dead zone on the sticks.** Every axis is sent on every poll,
  ...
  send on change only. Whether this pad needs one is a question for a
  pad.
